...
export default function UsersPage() {
  ...
  async function load() {
    try {
      ...
      notify({ tone: "danger", title: "Could not load users", message: getErrorMessage(err) });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveUser(event: React.FormEvent) {
    ...
    try {
      ...
      notify({ tone: "success", title: selectedUser ? "User updated" : "User created" });
      setUserModalOpen(false);
      setSelectedUser("");
      await load();
    } catch (err: any) {
      notify({ tone: "danger", title: "Could not save user", message: getErrorMessage(err) });
    } finally {
      setSaving("");
    }
  }
  ...
  async function deleteUser(user: any) {
    ...
    try {
      ...
      notify({ tone: "danger", title: "Could not delete user", message: getErrorMessage(err) });
    }
  }
  ...
  async function savePartner(event: React.FormEvent) {
    ...
    try {
      ...
      notify({ tone: "success", title: selectedPartner ? "Partner updated" : "Partner created" });
      setPartnerModalOpen(false);
      setSelectedPartner("");
      await load();
    } catch (err: any) {
      notify({ tone: "danger", title: "Could not save partner", message: getErrorMessage(err) });
    } finally {
      setSaving("");
    }
  }
  ...
  async function deletePartner(partner: any) {
    ...
    try {
      ...
      notify({ tone: "success", title: "Partner deleted" });
      await load();
    } catch (err: any) {
      notify({ tone: "danger", title: "Could not delete partner", message: getErrorMessage(err) });
    }
  }
  ...
  return (
    <RequireAuth>
      <PageHeader title="Users" description="Manage internal admin users, solution partners, and partner access to tenants, licenses, and configs." />
      <div className="page-stack">
        ...
        <div className="panel">
          <PanelHeader title="Admin portal users" description="Partner admins inherit access from their assigned solution partner." actions={<IconAction label="New user" tone="primary" onClick={createUser}><Plus size={16} /></IconAction>} />
              ...
              <table className="table">
                ...
                <tbody>{users.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td className="row actions"><IconAction label="Edit user" onClick={() => editUser(user)}><UserRound size={14} /></IconAction><IconAction label="Delete user" tone="danger" onClick={() => deleteUser(user)}><Trash2 size={14} /></IconAction></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
        ...
        <div className="panel">
          <PanelHeader title="Solution partners" description="Assign tenants and partner admins to the same partner to grant access." actions={<IconAction label="New partner" tone="primary" onClick={createPartner}><Plus size={16} /></IconAction>} />
              ...
              <table className="table">
                ...
                <tbody>{partners.map((partner) => (
                  <tr key={partner.id}>
                    ...
                    <td className="row actions"><IconAction label="Edit partner" onClick={() => editPartner(partner)}><Building2 size={14} /></IconAction><IconAction label="Delete partner" tone="danger" onClick={() => deletePartner(partner)}><Trash2 size={14} /></IconAction></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
      ...
      >
        <form id="admin-user-form" onSubmit={saveUser} className="form-stack">
          <FormSection title="User identity" description="The person who signs in to the admin portal.">
          ...
          <FormSection title="Access scope" description="Role and partner assignment determine which tenants, keys and configs are visible.">
            <div className="grid two">
          ...
          <FormSection title="Password" description={selectedUser ? "Leave blank unless you want to replace the current password." : "Set the initial sign-in password."}>
      ...
      >
        <form id="partner-form" onSubmit={savePartner} className="form-stack">
          <FormSection title="Partner identity" description="Solution partner details used when assigning users, tenants, keys and configs.">
            <div className="grid two">
